              ...
              <td className="px-2 py-1.5 text-stone-600 dark:text-stone-400">
                <div className="group relative max-w-[260px]">
                  <div className="truncate cursor-help">
                    {response.error ? <span className="text-rose-500">{response.error}</span> : response.finalOutput || <span className="text-stone-400">(empty)</span>}
                  </div>
                  {(response.finalOutput || response.error) && (
                    <div
                      className="invisible absolute right-0 top-full z-50 mt-1 hidden w-[420px] max-w-[80vw] rounded-md border border-stone-300 bg-white p-3 text-[11px] leading-relaxed text-stone-800 shadow-lg group-hover:visible group-hover:block dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100"
                      role="dialog"
                    >
                      <div className="mb-1 flex items-center justify-between text-[9px] uppercase tracking-[0.16em] text-stone-400 dark:text-stone-600">
                        <span>{LABEL[mode]} · full output</span>
                        <span className="tabular-nums">{(response.finalOutput ?? response.error ?? "").length} ch</span>
                      </div>
